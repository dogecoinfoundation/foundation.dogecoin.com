...
# Sym links created are kept to be removed
# into this array to be removed after the build process
SYM_LINKS=()

# Iterates on all the master files, then iterates on all
# the locales to check if the master file has a counterpart
# for that locale. If not we create a symlink.
...
# useful to avoid poluting the repo when running the command locally
